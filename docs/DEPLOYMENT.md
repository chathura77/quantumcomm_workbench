...
- The production URL is `https://quantum-workbench.sarathchandra.com`; leave `QUANTUMCOMM_BASE_PATH` unset or empty for this true subdomain deployment. For `https://www.sarathchandra.com/quantumworkbench`, set `QUANTUMCOMM_BASE_PATH=/quantumworkbench` before `npm run build` and route that path prefix to the Next.js server.
